  ...
  <div
    ...
    :style="
      `height: ${(showFlattedTreeList.length > showNumber
        ? showNumber
        : showFlattedTreeList.length) * itemHeight}px;`
    "
  >
    <div
    ...
    >
      <div
        v-for="(item, index) in useVirtualScroll
          ? activeList
          : showFlattedTreeList"
        :key="index"
        class="scroll-item"
      ...
      >
        <div class="inner-box" :style="`padding-left: ${item.level * 18}px;`">
          <span class="arrow" :class="{ 'not-open': !item.isExpand }">
            <i
              v-if="
                item[currentProps.children] &&
                  item[currentProps.children].length > 0
              "
          ...
          <span
            class="lucky-tree-label"
            :class="{ 'lucky-tree-label-checked': ifCurrentChecked(item) }"
  ...
  methods: {
      ...
      function travelTree(tree, level) {
        tree.forEach(item => {
          ...
          if (
            ...
            travelTree(item[that.currentProps.children], level + 1)
        })
      }
      travelTree(data, 0)
      return resList
    ...
    toggleExpand(item) {
      ...
      if (
        ...
        this.setTreeStatus(item[this.currentProps.children], !isExpand)
    },
    // 处理相应节点状态
    setTreeStatus(children, status) {
      const travel = list => {
        list.forEach(child => {
          child.isShow = status
          if (
            child[this.currentProps.children] &&
            child[this.currentProps.children].length
          ) {
            if ((status && child.isExpand) || !status)
              travel(child[this.currentProps.children])
          }
        })
      }
      travel(children)
