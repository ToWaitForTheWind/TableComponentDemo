<template>
    <div class="cascader-panel">
        <div
            class="cascader-data small-scroll-bar"
            :style="`${list.length > currentConfig.dropdownMaxLength ? 'width: 139px;' : ''} height: ${27 * currentConfig.dropdownMaxLength}px;`"
        >
            <template v-for="(item, index) in list">
                <div
                    v-if="!item[currentProps.delete]"
                    class="cascader-item"
                    :class="[{ 'is-checked': ifChecked(item), 'is-disabled': isDisabled(item), 'is-hover': isHover(item) }, item[currentProps.className]]"
                    :key="index"
                ...
                >
                    <div class="data-label">
                        <slot
                            ...
                            :listItem="item"
                        >
                            <span v-html="filterShowName(item[currentProps.label], searchValue)"></span>
                        </slot>
                    </div>
                    <i
                        v-if="showArrow(item)"
...
export default {
    name: 'CascaderPanel',
    components: {},
    inject: ['currentConfig', 'currentProps', 'filterShowName', 'toGetNodeParents'],
    props: {
    ...
    methods: {
        ...
        handleClickItem (item) {
            ...
            if (this.currentConfig.trigger === 'click' && item[this.currentProps.children] && item[this.currentProps.children].length) {
        ...
        toTriggerNext (item) {
            ...
            if (item[this.currentProps.children] && item[this.currentProps.children].length) {
                this.nextList = item[this.currentProps.children];
            } else this.nextList = [];
        },
        handleSelect (item) {
            if (this.isDisabled(item) || item[this.currentProps.canNotSelect]) return;
            if (item[this.currentProps.children] && item[this.currentProps.children].length) return;
            this.currentCheckedItem = item;
            this.toEmitChecked(item, this.level);
        },
        ifChecked (item) {
            let res = false;
            if (item[this.currentProps.value] == this.currentCheckedItem[this.currentProps.value]) res = true;
            return res;
        },
        isHover (item) {
            let res = false;
            if (item[this.currentProps.value] == this.currentHoverItem[this.currentProps.value]) res = true;
            return res;
        },
        isDisabled (item) {
            ...
            this.toGetNodeParents(item[this.currentProps.value], this.allData).forEach(node => {
                if (node[this.currentProps.disabled]) res = true;
            });
            return res;
        },
        showArrow (item) {
            return item[this.currentProps.children] && item[this.currentProps.children].length;
        },
